<?php
  ...
  */
	...
	#@ Do search
	/*
	$sql = 'SELECT * FROM usr LIKE "%'.$search_engine['options']['input'].'%" 
...
' ;
	...
	for ( $i = 1 ; $i < count($country_list) ; $i++ ) {
		...
		$id 			= "capitals_".trim($elements[0]) ;
		$common_name 	= trim($elements[1]) ;
		$formal_name	= trim($elements[2]) ;
		$capital	 	= trim($elements[6]) ;
		$code		 	= trim($elements[11]) ;
		$currency		 = trim($elements[7]) ;
		// Search on elements
		if ( preg_match('/'.$search_engine['options']['input'].'/i', $capital) ) {
			// To display
			// ini $results['value'] 		= utf8_encode($capital). ' ('.utf8_encode($code).')';
			//ini $results['info'] 		= utf8_encode($common_name) ;	
			//$results['currency'] 	= utf8_encode($currency) ;	
		
			// Store result
			// ini $search_engine['results'][$searchCat_name][$id] = $results ;
		}
	}
	// $db['default']['hostname'] = 'localhost:3434';
// $db['default']['username'] = 'root';
// $db['default']['password'] = 'kebersamaan';
// $db['default']['database'] = 'dbarofah';
define(BASEPATH,0);
error_reporting(1);
include ($_SERVER['DOCUMENT_ROOT'].'/application/config/database.php');
$a = ($db['online']);
$con = mysql_connect($a['hostname'],$a['username'],$a['password']);
if (!$con)
{
	die('Could not connect: ' . mysql_error());
}
mysql_select_db($a['database'], $con) or die("ERROR");


   
    $cari=$search_engine['options']['input'];
    $sql=" SELECT * FROM data_jamaah WHERE nama_jamaah like '%".$cari."%' OR no_porsi ='%".$cari."%'";
    $q=mysql_query($sql);

	//$search_engine['results'][$searchCat_name][$res['id_jamaah']] = array('Nama Jamaah','Nomor Porsi','Alamat KTP','Telepon'); ;
    while($res=mysql_fetch_array($q)){
		$results['value'] 		=  $res['nama_jamaah'].' / '.$res['tahun'];
		$results['nama'] 		=  $res['nama_jamaah'];
		$results['porsi'] 		=  $res['no_porsi'];
		$results['info'] 		=  'Porsi: '.$res['no_porsi'];
		$results['alamat'] 		=  $res['alamat_ktp'];
		$results['tahun'] 		=  $res['tahun'];
		$results['telepon'] 		=  $res['no_tlp'];
		$search_engine['results'][$searchCat_name][$res['id_jamaah']] = $results ;
		}
?>