...
$pdf->addText(285, 540, 10,'<b>PEMBAYARAN JAMAAH KBIH AN NAMIROH MOJOKERTO</b>');// Garis atas untuk header
...
$pdf->addObject($all, 'all');
	...
	$tgl_bayar=$_GET['tgl'];
$p = $_GET['p'];$pieces = explode("/", $p);$bulan=$pieces[0];$th=(int)$pieces[1];
	
	$sql = "SELECT * FROM semua_data1 ";$sql.= "WHERE year(tgl_bayar)='".$th."' and month(str_to_date('$bulan','%b')) =month(tgl_bayar)";
...
$cols = array('no'=>"<b>No</b>", 'penyetor'=>'<b>Penyetor</b>', 'nama'=>'<b>Atas Nama</b>', 'tgl'=>'<b>Tanggal</b>', 'nominal'=>'<b>Nominal (Rp)</b>', 'ket'=>'<b>Keterangan</b>');
	
$pdf->ezTable($data,$cols,'',array('xPos'=>50,'xOrientation'=>'right','width'=>770,'cols'=>array(
'no'=>array('justification'=>'right')
,'penyetor'=>array('width'=>150)
