...
<div class="container">
    ...
    <form id="contact" method="POST" action="<?= $urlSave ?>">
        <input required type="hidden" name="id_koper_jamaah" value="<?= $idKoperJamaah ?>">
        <!-- <input required type="hidden" name="id_jamaah" value="<?= $idJamaah ?>"> -->
        <!-- <input required type="hidden" name="id_paket" value="<?= $idPaket ?>"> -->

        <div style="text-align: left;">
            <div class="line">
                <label style="text-align: left;" for="name">Input Tanggal </label>
            </div>
            <div class="line">
                <input style="margin-right: 30px; vertical-align: baseline;" required type="date" name="tanggal_keluar" id="tanggal_keluar" value="<?= @$koperJamaah->tanggal_keluar ?>">
            </div>
        </div>

        <div style="text-align: left;">
            <div class="line">
                <label style="text-align: left;" for="name">Penerima / No Resi </label>
            </div>
            <div class="line">
                <input style="margin-right: 30px; vertical-align: baseline;" required type="text" name="nomor_resi" id="nomor_resi" value="<?= @$koperJamaah->nomor_resi ?>">
            </div>
        </div>
        ...
        <div style="text-align: left;margin-top:40px;">
            <div class="line">
                ...
                <label style="text-align: right;" for="name">Qty :</label>
            </div>
        </div>
        <?php
        foreach (@$barangKeluar as $barang) : ?>
            <div class="" style="margin-left:0px; margin-top:10px;">
                <div class="line">
                    ...
                    <input style="margin-left: 30px; vertical-align: baseline;" required <?= $barang->is_active ? "" : "disabled" ?> class="qty" type="number" name="qty_<?= $barang->id ?>" id="qty_<?= $barang->id ?>" value="<?= $barang->jumlah > 0 ? $barang->jumlah : 1 ?>">
                </div>
            </div>
        <?php endforeach ?>
