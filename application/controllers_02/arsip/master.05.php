...
class Master extends CI_Controller {
	...
	function group_level(){
		$this->crud->set_table('group_level')->unset_add()->unset_delete()->unset_edit()->columns('id','nama','keterangan')->order_by('id');
	...
	function perhitungan(){
		...
		$this->crud->columns('keberangkatan', 'jual','jumlah_pax', 'tiket', 'visa', 'perlengkapan', 'handling', 'operasional', 'baksis', 'bus', 'guide','lama_guide' ,'hotel_makkah', 'hotel_madinah', 
	...
	'jumlah_hari_makkah', 'jumlah_hari_madinah','jumlah_free', 'laba','kurs')->callback_column('jual',array($this,'_callback_jual'));
	...
	public function _callback_jual($value, $row)
	{
	  ...
	  $real = $row->handling+$row->operasional+$row->baksis/$row->jumlah_pax+$row->bus/$row->jumlah_pax+$row->guide*$row->lama_guide/$row->jumlah_pax+$row->hotel_makkah*$row->jumlah_hari_makkah/4+$row->hotel_madinah*$row->jumlah_hari_madinah/4;
	  $a.="+SAR $real <br>";
	...
	function akses($group = 0,$kategori = 0){
	    if($group == 0 || $kategori == 0){
	        $this->crud->set_table('pilihan_menu1')->unset_add()->unset_delete()->unset_edit()->columns('nama','kategori');
            ...
            $this->crud->set_table('page_akses')->columns('link','menu','aktif')->field_type('kategori', 'hidden', $kategori)->field_type('group', 'hidden', $group)->where(array('kategori'=>$kategori,'group'=>$group));
            //
	...
	function agen(){
		...
		$this->crud->set_theme('datatables');
		$this->crud->set_subject('Data Agen Umroh')->display_as('id','Nomor Agen')->columns('id','nama','alamat','telepon','email','hp','keterangan','leader')->unset_delete()->where('data_jamaah_agen.pangkat',0);	
	...
	function agen_(){
		...
		$this->crud->set_table('data_jamaah_agen');
		$this->crud->set_theme('datatables');
		$this->crud->set_subject('Data Agen Umroh')->display_as('id','Nomor Agen')->columns('id','nama','alamat','telepon','email','hp','keterangan')->unset_delete()->where('data_jamaah_agen.pangkat',0)->where('leader',$fk);	
		$this->crud->field_type('pangkat','hidden',0)->callback_column('nama',array($this,'_urle'));;//0 agen, 1 leader
		$this->show();
	}
	function leader(){
		$crud = new grocery_CRUD();
		$this->crud->set_table('data_jamaah_agen')->columns('id','nama','alamat','telepon','email','hp','keterangan')->fields('nama','alamat','telepon','email','hp','keterangan','pangkat');
		$this->crud->set_theme('datatables');
	...
	function jamaah_p(){
		...
		$this->crud->set_subject('Pilih Paket Umroh')->unset_edit()->unset_delete()->columns('estimasi_keberangkatan','Program');
	...
	function jamaah($paket=0){
		...
		$this->crud->columns('nama_jamaah','tgl_lahir','no_ktp','agen','hp_jamaah','alamat');
		// $this->crud->set_relation_n_n('paket','transaksi_paket','data_jamaah_paket','jamaah','paket_umroh','estimasi_keberangkatan');
	...
	//	$this->crud->set_relation('hotel','data_hotel','nama');
	    ...
	    $this->crud->columns('estimasi_keberangkatan','Program','Penerbangan','hotel_makkah','hotel_madinah','harga','pembimbing','harga_dolar','sisa_kursi')->unset_delete()->set_relation('pembimbing','data_jamaah_pembimbing','nama');
		$this->show();
	}
	function paket_($arsip='1'){
		...
		$this->crud->set_relation('Penerbangan','data_maskapai','nama');
		$this->crud->set_subject('Data Paket Umroh');
	    $this->crud->columns('estimasi_keberangkatan','Program','Penerbangan','pembimbing','hotel_makkah','hotel_madinah','harga','sisa_kursi','KET')->fields('estimasi_keberangkatan','Program','Penerbangan','hotel_makkah','hotel_madinah','pembimbing','harga','sisa_kursi','KET')->unset_delete()->set_relation('pembimbing','data_jamaah_pembimbing','nama');
		$this->show();
	}
	function pembimbing(){
		$this->crud->set_table('data_jamaah_pembimbing');
		$this->crud->set_subject('Data Pembimbing');
	    $this->crud->columns('nama','alamat','pendidikan','sertifikat')->unset_delete();
		$this->show();
	}
