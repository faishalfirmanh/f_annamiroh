...
class Transaksi extends CI_Controller {
    ...
    function pakete(){
        $this->crud->set_table('data_jamaah_paket')->columns('estimasi_keberangkatan','Program','tanggal_keberangkatan','total_seat','harga','Penerbangan','rute')->unset_edit()->display_as('estimasi_keberangkatan','Paket')->unset_delete()->unset_add()->set_subject('Paket')->where('KET','AKTIF')->where('TAMPIL','YA');
			$this->crud->set_relation('Penerbangan','data_maskapai','nama');
	...
	function pembayaran($paket=0,$jamaah =0){
		...
		if($paket==0){
			$this->crud->set_table('data_jamaah_paket')->columns('estimasi_keberangkatan','total_seat','tanggal_keberangkatan','Program','harga','harga_dolar')->unset_edit()->display_as('estimasi_keberangkatan','Pilih Paket')->unset_delete()->unset_add()->set_subject('Pilih Paket')->where('KET','AKTIF')->where('TAMPIL','YA');
			$this->db->select('paket_umroh');
			$query = $this->db->get('transaksi_paket');
			foreach ($query->result() as $row)
			{
