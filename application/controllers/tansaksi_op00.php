...
class Transaksi_Op extends CI_Controller {
	...
	public function _callback_webpage_url($value, $row)
    {
		...
		return "<a href='".site_url('transaksi_op/pembayaran/'.$row->id)."'target='_blank'>$value-$jumlae orang</a>";
    }
	...
	function pembayaran($paket=0,$jamaah =0){
		if($paket==0){
			...
			$this->crud->set_table('data_jamaah_paket')->unset_read()->columns('estimasi_keberangkatan','tanggal_keberangkatan','Program','harga')->fields('estimasi_keberangkatan','Program','harga','KET')->display_as('estimasi_keberangkatan','Pilih Paket')->unset_delete()->unset_add()->set_subject('Pilih Paket')->order_by('tanggal_keberangkatan','DESC');
	...
	function arsip_pembayaran($paket=0,$jamaah =0){
		...
		elseif($jamaah==0){
			...
			$this->crud->set_table('transaksi_paket')->set_subject('Pembelian paket umroh '.$s)->unset_read()->columns('jamaah','harga','kredit','kekurangan','debet','saldo','kode','agen');
	...
	*/
	...
	function histori($id=0){
		...
		$kurang = $harga - $kredit;
		$this->crud->set_subject("Transaksi Kredit $jamaah | Paket :$paket | Harga:$harga | Pembayaran:$kredit | Kekurangan: ".$kurang."<br>Transaksi Debet : $debet")
		->set_top("Transaksi Kredit $jamaah | Paket :$paket | Harga:$harga | Pembayaran:$kredit | Kekurangan: ".$kurang."<br>Transaksi Debet : $debet")->unset_add();
