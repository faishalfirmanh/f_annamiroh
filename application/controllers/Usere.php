...
class Usere extends CI_Controller {
	...
	function shortcut(){
        $this->crud = new grocery_CRUD();
		$this->crud->set_table('user_shortcut')->unset_read()->columns('link','menu','level','urutan')->set_relation('level','group_level','nama')->order_by('level')->order_by('urutan');
	...
	function index(){
		...
		$this->crud->set_subject('Paket Pembelian')->unset_read()->columns('kode','harga','kredit')->where(array('jamaah'=>$fk))->unset_edit()->unset_add()->unset_delete();
		$this->show();
	...
	function agen_leader($fk){
		...
		$this->crud->set_table('transaksi_paket');
		$this->crud->set_subject('Paket Pembelian')->unset_read()->columns('jamaah','kode','harga','kredit','kekurangan')->where(array('transaksi_paket.agen'=>$fk))->unset_edit()->unset_add()->unset_delete()->set_relation('jamaah','data_jamaah','{nama_jamaah}-{no_ktp}');
		$this->show();
