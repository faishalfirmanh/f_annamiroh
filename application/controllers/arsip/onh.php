<?php
class Onh extends CI_Controller {
	...
	function update_debet_proses(){
		...
		$data['form_action'] = site_url('onh/update_debet_proses');
		$data['isi'] = 'onh/onh_form_debet';
		//session
	...
	function debet_proses(){
		...
		$data['nm_total'] = 'Saldo';
		$data['isi'] = 'onh/onh_form_debet';
		$data['form_action'] = site_url('onh/debet_proses');
		//session
		$id_jamaah = $this->session->userdata('id_jamaah');
		//Data Jamaah
		$jamaah = $this->Jamaah_model->get_jamaah_by_id($id_jamaah);
	...
	function debet(){
		...
		if ($num_rows > 0){
			...
			$config['base_url'] = site_url('onh/debet');
			...
			foreach ($onhs as $onh){
				...
				$onh->nama_admin,
				anchor ('onh/debet/'.$offset,'print',array('class' => 'print','onclick'=>"MM_openBrWindow('".base_url()."pdf/kwitansi.php?t=debet_onh&id=".$onh->id_pembayaran."','','scrollbars=yes,width=900,height=600');")), 
				anchor ('onh/update_debet/'.$onh->id_pembayaran,'edit',array('class' => 'update')).' '.
				anchor('onh/del_debet/'.$onh->id_pembayaran,'hapus',array('class'=> 'delete','onclick'=>"return confirm('Anda yakin akan menghapus data ini?')")));
			}
		...
		//
		$data['link'] = array('link_add' => anchor('onh/add_debet/','tambah data', array('class' => 'add')));
		$this->load->view('template', $data);
	}
	...
	function update_kredit_proses(){
		...
		$data['main_view'] = 'transaksi_template';
		$data['data_jamaah'] = 'jamaah/jamaah_data_view';
		$data['form_action'] = site_url('onh/update_kredit_proses');
		$data['isi'] = 'onh/onh_form_kredit';
		//session
		$id_jamaah = $this->session->userdata('id_jamaah');
		//Data Jamaah
		$jamaah = $this->Jamaah_model->get_jamaah_by_id($id_jamaah);
	...
	function kredit($offset = 0){
		...
		$data['default']['nama_jamaah'] = $rjamaah['nama_jamaah'];
		//Total 
		$total = $this->Onh_model->total_kredit($id_jamaah);
		$row = $total->row_array(); 
		...
		$onhs = $this->Onh_model->kredit($id_jamaah, $this->limit, $offset)->result();
		$num_rows = $this->Onh_model->count_kredit($id_jamaah);
		
		if ($num_rows > 0){
			$data['default']['num'] = $num_rows;
			//$data['message'] = 'ada transaksi!';
			$config['base_url'] = site_url('onh/kredit');
			$config['total_rows'] = $num_rows;
			$config['per_page']	= $this->limit;
			$config['uri_segment'] = $uri_segment;
			$this->pagination->initialize($config);
			...
			foreach ($onhs as $onh){
				...
				$onh->nama_admin,
				anchor ('onh/kredit/'.$offset.'#','print',array('class' => 'print','onclick'=>"MM_openBrWindow('".base_url()."pdf/kwitansi.php?t=onh&id=".$onh->id_pembayaran."','','scrollbars=yes,width=900,height=600');")), 
				anchor ('onh/update_kredit/'.$onh->id_pembayaran,'edit',array('class' => 'update')).' '.
				anchor('onh/del_kredit/'.$onh->id_pembayaran,'hapus',array('class'=> 'delete','onclick'=>"return confirm('Anda yakin akan menghapus data ini?')")));
			}
		...
		//
		$data['link'] = array('link_add' => anchor('onh/add_kredit/','tambah data', array('class' => 'add')));
	...
	function umroh_kredit($modul=1,$offset = 0){
		...
		//
		$data['link'] = array('link_add' => anchor("onh/add_kredit_umroh/$modul",'Tambah data', array('class' => 'add')));
		$this->load->view('template_umroh', $data);
	...
	function add_kredit(){
		...
		$data['main_view'] = 'transaksi_template';
		$data['data_jamaah'] = 'jamaah/jamaah_data_view';
		$data['form_action'] = site_url('onh/kredit_proses');
		$data['isi'] = 'onh/onh_form_kredit';
	...
	function add_kredit_umroh($modul=1){
		...
		$data['main_view'] = 'transaksi_template';
		$data['data_jamaah'] = 'jamaah/jamaah_data_view_umroh';
		$data['form_action'] = site_url("onh/kredit_proses_umroh/$modul");
		$data['isi'] = 'onh/onh_form_kredit_umroh';
	...
	function kredit_proses_umroh($modul = 1){
		...
		if ($this->form_validation->run() == TRUE){
			...
			redirect("onh/umroh_kredit/$modul");
		}
	...
	function get_last_ten_onh($offset = 0){
		...
		if ($num_rows > 0){
			$data['default']['num'] = $num_rows;
			//$data['message'] = 'ada transaksi!';
			$config['base_url'] = site_url('onh/get_last_ten_onh');
			$config['total_rows'] = $num_rows;
			$config['per_page']	= $this->limit;
			$config['uri_segment'] = $uri_segment;
			$this->pagination->initialize($config);//.
