...
class Log extends CI_Controller {
	...
	function get_last_ten_log($tabel,$offset = 0){
		...
		$crud->set_table($tabel)->unset_add()->unset_delete()->unset_edit();
		$crud->set_subject('Data Log '.substr($tabel,0,-4));
		
		$this->load->view('template_new1', $crud->render());
	}
	function get_last_ten_logs($tabel,$offset = 0){
		$data['title'] = substr($tabel,0,-3);
		$data['nama_menu'] = 'Log '.substr($tabel,0,-3);
		$data['menu_kiri'] = 'log/log_left';
