...
class Transaksi_Op extends CI_Controller {
	...
	function manifest($paket = 0){
	    ...
	    $state = $this->crud->getState();
    		...
    		$d= $this->db->query("select id_jamaah,nama_jamaah,title,place,age,place,issued, passport,expired,office,no_tlp,hp_jamaah,alamat_jamaah,tgl_lahir from data_jamaah");
    		// print_r($d);
    		foreach($d->result() as $row){
    			$this->jamaahnya[$row->id_jamaah]=array($row->nama_jamaah,
    			$row->title,$row->age, $row->passport,$row->place,$row->issued,$row->expired,$row->office,$row->tgl_lahir,$row->no_tlp,$row->hp_jamaah,$row->alamat_jamaah
    			);
    		}
    		// print_r($this->jamaahnya);
    		$r = $this->get('data_jamaah_paket','id',$paket,'harga');
    		$this->r = $r;
    		if($r <> null) $s.='Rp.'.$this->main_model->uang($r);
    		$this->grocery_crud->callback_add_field('harga',array($this,'harga_field_callback_1'))->unset_edit();
    		$this->crud->set_table('transaksi_paket')->set_subject('Manifest Jamaah '.$s)->columns('jamaah','kekurangan','title','age', 'passport','place','expired','issued','office','lahir','tlp','hp','alamat');
    		...
    		$this->crud->callback_column('lahir',array($this,'edit_callback8'));
	...
	function update_kekurangan_on_harga($post_array,$primary_key)
    {
        ...
        $kredit = $post_array['kredit'];
        $debet = $post_array['debet'];
	...
	function pembayaran($paket=0,$jamaah =0){
		if($paket==0){
			...
			$this->crud->set_table('data_jamaah_paket')->columns('estimasi_keberangkatan','tanggal_keberangkatan','Program','kekurangan')->fields('estimasi_keberangkatan','Program','harga','KET')->display_as('estimasi_keberangkatan','Pilih Paket')->unset_delete()->unset_add()->set_subject('Pilih Paket')->order_by('tanggal_keberangkatan','DESC');
		...
		elseif($jamaah==0){
			...
			$this->grocery_crud->callback_add_field('harga',array($this,'harga_field_callback_1'))->unset_edit();
			$this->crud->set_table('transaksi_paket')->set_subject('Pembelian paket umroh '.$s)->columns('jamaah','harga','kredit','debet','kode','agen');
		...
		//	$this->crud->callback_column('saldo',array($this,'_rupiah'));
			...
			$this->crud->unset_edit()->unset_add();
			$this->crud->set_table('transaksi_paket')->set_subject('Pembelian paket umroh '.$s)->columns('jamaah','harga','kredit','debet','kode','agen');
			$this->crud->field_type('kode', 'readonly')->set_relation('jamaah','data_jamaah','{nama_jamaah}-{no_ktp}-{alamat_jamaah}-{no_tlp}','nama_jamaah <> ""');
            // $this->crud->callback_column('detil',array($this,'__jamaah'));
	...
	*/
	function debet($id=0){
		...
		$this->crud->columns('jenis_transaksi','keterangan','tanggal','tanggal_transfer','kredit','debet','teller');
