...
class Web extends CI_Controller {
	...
	function post($tipe=0){
	    if($tipe == 0 ){
	        $this->crud->set_table('web_tipe_pos')->unset_add()->unset_delete()->unset_edit()->columns('tipe');
	       ...
	       $d = $this->get('web_tipe_pos','id',$tipe,'tipe');
            $this->crud->set_table('web_konten')->columns('judul','isi','file','waktu_tayang','tampil')->where(array('kategori'=>$tipe))->field_type('kategori', 'hidden', $tipe);
    		$this->crud->set_subject("Data Konten $d")->unset_texteditor('judul','full_text')->unset_texteditor('isi','full_text')->display_as('file','File / Foto')->set_relation('tampil','web_status_post','status');
