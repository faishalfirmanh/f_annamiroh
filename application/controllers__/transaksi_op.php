...
class Transaksi_Op extends CI_Controller {
	...
	public function _callback_webpage_url($value, $row)
    {
		...
		return "<a href='".site_url('transaksi_op/pembayaran/'.$row->id)."'>$value-$jumlae orang</a>";
    }
	...
	function pembayaran($paket=0,$jamaah =0){
		...
		if($paket==0){
			$this->crud->set_table('data_jamaah_paket')->columns('estimasi_keberangkatan','Program','harga','harga_dolar')->unset_edit()->display_as('estimasi_keberangkatan','Pilih Paket')->unset_delete()->unset_add()->set_subject('Pilih Paket');
	...
	{
		return "<a href='".site_url('transaksi_op/histori/'.$row->id)."' target='_blank'>$value</a>";
	}
	public function __debet($value, $row)
	{
		return "<a href='".site_url('transaksi_op/debet/'.$row->id)."'  target='_blank'>$value</a> <a href='".site_url('transaksi_op/debet/'.$row->id)."/add'  target='_blank'>+</a>";
	}
	public function __kredit($value, $row)
	{
		return "<a href='".site_url('transaksi_op/kredit/'.$row->id)."'  target='_blank'>$value</a> <a href='".site_url('transaksi_op/kredit/'.$row->id)."/add'  target='_blank'>+</a>";
	}
	...
	*/
	...
	function kredit($id=0){
		...
		$this->crud->fields('id_transaksi_paket','jenis_transaksi','tanggal','tanggal_transfer','kredit','keterangan','teller')->unset_edit()->unset_delete();
	    $this->show();
	}
